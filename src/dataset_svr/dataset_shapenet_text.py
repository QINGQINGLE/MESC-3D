...
def get_prompt_text(category: list, label_map: dict) -> list:
    ...
    # 查找category中的每个元素对应的label_map中的值
    if cat_str in label_map:
        prompt_text.append(label_map[cat_str])
    else:
        # 如果category中的某个元素在label_map中没有对应值，可以选择跳过或返回一个默认值
        prompt_text.append('Unknown')  # 或者可以用 None 或其他占位符
    return prompt_text
class ShapeNet(data.Dataset):
    ...
    def __init__(self, opt, train=True):
        ...
        if not opt.demo:
            ...
            # Load taxonomy file
            self.taxonomy_path = opt.taxonomy_path
            self.caption_path = "Path/captions.json"
    ...
    def init_singleview(self):
        ...
        self.label_map = dict({'02691156':'airplane','02828884':'bench','02933112':'cabinet','02958343':'car','03001627':'chair',
    ...
    def __getitem__(self, index):
        ...
        # prompt loading
        try:
            cap = self.captions[return_dict["category"]][return_dict["name"].split(".")[0]]
        except KeyError as e:
            category = return_dict["category"]
            prompt_text = get_prompt_text(category,self.label_map)
            cap = prompt_text * 10
            # print(f"cap")

        # cap = self.captions[return_dict["category"]][return_dict["name"].split(".")[0]]
        return_dict["caption"] = cap
        return return_dict
...
if __name__ == '__main__':
    ...
    opt = {"normalization": "UnitBall", "class_choice": ["airplane"], "SVR": True, "sample": True, "npoints": 2500,
           "shapenet13": True}
