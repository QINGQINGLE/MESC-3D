...
class SMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            if i ==0:
                patch_embed = Head(head_conv, embed_dims[i])# STEM
            else:
                patch_embed = OverlapPatchEmbed(img_size=img_size if i == 0 else img_size // (2 ** (i + 1)), # H/4 H/8 H/16 H/32 
                                            patch_size=3,
                                            stride=2,
    ...
    def forward_features(self, x):
        ...
        return feature,x.mean(dim=1)

    def forward(self, x):
        feature,x1 = self.forward_features(x)
...
class DWConv(nn.Module):
    def __init__(self, dim=768):
        ...
        self.dwconv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True, groups=dim)  # bushi shendu kefenli juanji
...
if __name__ == '__main__':
    ...
    model = smt_l()
    model = model.cuda()
    ckpt_path = '/Path/checkpoints/smt_large_22k_384_ft.pth'
    ckpt = torch.load(ckpt_path)
    model.load_state_dict(ckpt['model'],strict=False)

    input = torch.rand(4, 3, 224, 224).cuda()
    feature,output = model(input)
    print(f'x: {feature[0].shape}')
