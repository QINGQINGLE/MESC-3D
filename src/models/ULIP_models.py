...
# from dataset_3d import cfg_from_yaml_file

from loss import losses_v2
...
class ULIP_WITH_IMAGE(nn.Module):
    ...
    def forward(self, pc, text=None, image=None):
        ...
        text_embed_all = torch.randn(1,2)
...
def get_loss_v2(args):
    return losses_v2.ULIPWithImageLoss()

def get_metric_names(model):
    return ['loss', 'ulip_loss', 'ulip_text_pc_sim', 'ulip_text_image_sim']
...
def ULIP_PointBERT(args):
    ...
    config_addr = 'Path/models/pointbert/PointTransformer_8192point.yaml'
...
def ULIP2_PointBERT_Colored(args):
    ...
    config_addr = 'Path/models/pointbert/ULIP_2_PointBERT_10k_colored_pointclouds.yaml'
    config = cfg_from_yaml_file(config_addr)
    point_encoder = PointTransformer_Colored(config.model, args=args)
    pc_feat_dims = 768
    # =====================================================================

    model = ULIP2_WITH_OPENCLIP(open_clip_model=None, point_encoder=point_encoder, pc_feat_dims=pc_feat_dims)
