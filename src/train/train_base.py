...
from network.base_model import AverageValueMeter,build_optimizer,save_model_2
import argparse
...
def train():
    ...
    if args.distributed:
        svrmodel = torch.nn.DataParallel(BaseModel(),device_ids=args.gpus,output_device=args.gpus[0])
        svrmodel.to(device)
    else: 
        svrmodel = BaseModel.to(device)
    
    optimizer,scheduler = build_optimizer(svrmodel,args)
    ...
    for epoch in tqdm(range(args.start_epoch,args.nepoch),desc='Training'):
        ...
        with tqdm(dataloader) as t:
                for batch_idx,data in enumerate(t):
                    ...
                    pred_points = svrmodel(images,partial_pcs)
                    # to(cuda)
                    ...
                    writer.add_scalar('MAE_netloss',net_loss.item(),n_itr)
                    writer.add_scalar('MAE_losst',loss_t.item(),n_itr)
        ...
        writer.add_scalar('MAE_CDL2', avg_cd_l2, epoch)
        writer.add_scalar('MAE_CDL1', avg_cd_l1, epoch)

        if epoch % args.epoch_interval_to_val == 0 or epoch == args.nepoch - 1:
            best_cd_l1, best_cd_l2 ,best_f1= val(svrmodel,loss_function,epoch,val_loss_meters,dataloader_test,best_epoch_losses,LOG_FOUT,Log_dir,best_cd_l1,best_cd_l2,best_f1)
    ...
    with torch.no_grad():
        with tqdm(dataloader_test) as tt:
            for i,data in enumerate(tt):
                ...
                partial_pcs = torch.tensor(sphere_points).to(torch.float32).unsqueeze(0).repeat(images.shape[0], 1, 1).to(device)
                pred_points = net(images,partial_pcs)
                pred_points = pred_points.transpose(2,1)
