...
from network.base_model import AverageValueMeter
...
def val():
    ...
    if args.distributed:
        svrmodel = torch.nn.DataParallel(BaseModel(args.model),device_ids=args.gpus,output_device=args.gpus[0])
        svrmodel.to(device)
        svrmodel.module.load_state_dict(ckpt['net_state_dict'])
        logging.info("%s's previous weights loaded." % args.model_name)
        svrmodel.module.eval()
    else:
        svrmodel = BaseModel(args.model).to(device)
        svrmodel.load_state_dict(ckpt['net_state_dict'])
        logging.info("%s's previous weights loaded." % args.model_name)
        svrmodel.eval()
